...
>
> **Last verified implementation:** 2026-09-07, Meetings Phase 7 package P7.6 (code complete; host
> execution pending). Previously 2026-09-04, package P7.1 — ready-anytime meeting
...
> Planner Phases 17–23 are complete; see [PLANNER.md](PLANNER.md). The API exposes 181 endpoints;
> the UI consumes 178. The remaining three are the signed provider webhook and the two long-standing
...
| **Endpoints** | **181** exposed (19 controllers; excludes the Angular-template `WeatherForecastController`) | **178 consumed.** The provider webhook is server-to-server; the 2 deliberate skips below remain |
...
| Phase 7 — hardening and rollout | 🟡 | 🟡 | **Deferral lifted 2026-09-04; sidebar entry restored.** Production now carries a complete valid LiveKit configuration and the media endpoint answers healthy over TLS; a real call is still unproven. **P7.1 done:** ready-anytime creation fixed, and `GET /admin/meetings/readiness` plus its admin panel make an unpropagated `LiveKit__*` deployment visible instead of surfacing as a join refusal. **P7.2–P7.5 done:** threat model and abuse review, declared capacity and concurrency, structured metrics/traces/logs with alert rules and runbooks, and the critical journey plus its denial paths driven end to end over real HTTP. **P7.7 done:** privacy/retention, support troubleshooting and backup/rotation/incident procedures documented, and the public privacy policy now covers meetings — including the honest statement that retention does not reach guest email addresses (proposed fix: P7.8). **P7.6 code complete (2026-09-07):** production TURN was UDP-only with no domain or TLS, so a participant on a network permitting only `443/tcp` had no path into a meeting at all; `dokploy.compose.yml` now serves TURN/TLS on 443 via Traefik SNI, `infra/meetings/turn-check.html` proves a relay-only join from a restrictive network, and `infra/meetings/ROLLOUT.md` carries provisioning, the configuration File Mount and the staged flag order. **Execution remains owner-gated and is the only thing left in Phase 7:** DNS, redeploy, certificate, a real restrictive-network `PASS`, the File Mount, and raising `Enabled` → `GuestsEnabled` → `RecordingEnabled` in stages. |
...
| **Meetings health (admin)** | `GET /admin/meetings/health` | ✅ | ✅ | ⬜ | **Meetings P7.4 (2026-09-05).** The rolling in-process signal window and every alert rule evaluated against it, with the runbook anchor for each. Counts and rule outcomes only — no meeting, participant, room or address — so the panel is safe to screenshot into a ticket. Rendered on `/admin/settings` as **Meetings health**. Per instance and reset on restart, both reported in the response. Not yet exercised against a deployed API |
...
| **Project plan import** | `POST /project/plan` | ✅ | ✅ | ⬜ | Landed 2026-09-03 in commits `d62ca61` (API) / `c3582ad` (UI) and **undocumented until 2026-09-04**. One transactional import of a whole project with tasks, subtasks, team and assignee resolution; the UI parses the file client-side and offers it on both the organization and member projects pages. **Two file formats since 2026-09-13**, both producing the identical request body: CSV (`@shared/utils/project-plan-csv`) and Markdown (`@shared/utils/project-plan-markdown`), the latter added because an assistant writes a Markdown plan far faster and more reliably than a spreadsheet. **No API change** — the endpoint never saw the file |
...
| Date | Side | Change |
|---|---|---|
| 2026-09-22 | **API** | **TaskFlow can send mail: self-hosted mailcow, two sending identities.** No API surface changed — **no route, no migration, no contract**, ledger unchanged at **182 total**. Recorded here because [SECRETS.md](SECRETS.md) has said since 2026-09-08 that *invites do not send yet* for want of SMTP, which silently capped organization invitations, email verification and meeting guest codes on **both** sides: the frontend's invitation and verify-email screens were complete and calling endpoints that composed a message and dropped it. `EmailSettings` still pointed at the `smtp.gmail.com` / `noreply@example.com` placeholder. **Two senders, split by blast radius rather than branding:** `EmailSender.Transactional` (`noreply@inksphere.space`) carries everything a user must act on — verification, sign-in and reset codes, org invitations, meeting invites and guest codes — and `EmailSender.Product` (`taskflow@inksphere.space`) carries the new thank-you and is replyable, so a spam complaint about friendly mail cannot stop a reset code reaching a locked-out user. `IEmailService.SendAsync` takes the sender with **no default**, so all six send sites now name their identity where the message is composed. **The thank-you did not previously exist** — `UserRegisteredEventHandler` sends the *verification* mail despite the repo calling it the welcome email — and it is tied to **verification, not registration**: `UserEmailVerifiedEvent` was already raised by `User.VerifyEmail()` with **no handler at all**. That placement is deliberate twice over: at registration the only thing the user needs is the verify link, and verification is the point the address is proven real, which matters on a domain with zero reputation. A failed thank-you logs and swallows — events dispatch after the commit, and nobody should be told a valid link failed over a courtesy email. **The non-obvious infrastructure detail is the host/address split:** mailcow serves both domains from one box and **`mail.inksphere.space` has no A record**, so the host is `mail.buildbykashyap.in` while the From addresses are `@inksphere.space`; matching the host to the address is the intuitive move and breaks all outbound mail. **Port 587, not 465:** `System.Net.Mail.SmtpClient` has no implicit-TLS mode, so 465 is open, looks safer, and hangs. Deliverability verified against `1.1.1.1`: MX, SPF, 2048-bit DKIM, DMARC and a PTR matching HELO all correct. mailcow hygiene applied — both mailboxes were named `inksphere` in the From line, now `TaskFlow`; `noreply@` quarantine notices `Daily` → `Never`. Vault went **35 → 40 secrets** (4 stale Gmail-era overwrites, 5 new); `EmailSettings__FromEmail`/`__FromName` are now dead keys. Backend **118/118**, build clean. **Two caveats accepted, not fixed:** DMARC is `p=none`, so nothing is enforced against a spoof; and the domain has **zero sending history**, so expect greylisting and spam-foldering from the large providers however correct the DNS — warm the volume up rather than batching. **Proven end to end 2026-09-22:** the startup config report named both app passwords with no `MISSING:` line — which, since the previous `report-config.sh` did not know those names, simultaneously proved the new build was running and the vault values had arrived — and `POST /auth/login-code/request` returned **200 in 2453ms**, which can only mean the send completed because the handler rethrows on failure. The code reached Gmail **and landed in spam**, exactly as the zero-reputation caveat predicted. This also proves the Docker hairpin: API and mailcow share a host, so the container reaches mailcow through its own public IP — the one path no local test can exercise. |
| 2026-09-13 | **API** | **Production was down for 50 minutes (09:52–10:43 UTC) — every endpoint 502 — and the Infisical cutover is now complete.** No API surface changed: **no route, no migration, no contract**, ledger unchanged at **182 total**. Recorded here because the frontend was fully blocked and this is the only doc that says so. **Two stacked faults, both configuration.** (1) The [SECRETS.md](SECRETS.md) cutover ran **step 5 before step 3**: Dokploy’s environment block was cleared down to the 11 `LiveKit__*` / `Meetings__*` variables while the four `INFISICAL_*` credentials had never been added, so `docker-entrypoint.sh` took its documented fallback into an almost-empty environment and startup died on the first database call (`The ConnectionString property has not been initialized`). The fallback worked exactly as designed — the step ordering is load-bearing, not advisory. (2) Once the credentials were added a **second, identical-looking fault** appeared: `apt-get install infisical` was **unpinned**, so that morning’s rebuild silently took CLI `0.43.132`, which fetches secrets over `/api/v4/secrets` — a route this self-hosted vault does not serve. It answered `404`, `infisical run` fetched nothing, and the API exec’d with an empty environment again. Pinned to **`0.42.6`**, the last release using `/api/v3/secrets/raw`, with `infisical --version` at build time so a wrong version fails the **build** rather than the **boot**. **Cutover step 4 is satisfied by construction:** the container environment now holds 15 variables, none of them `ConnectionStrings__*` or `JwtSettings__*`, yet the API reports every one present — they can only have come from the vault. **Open risk, worse than the outage:** the vault’s image was built **2025-08-08** and its 2026-09-08 restart reused it, so it is 13 months stale; with Dokploy’s copies cleared it is now the **only** copy of all 35 production secrets and the host still has no backups. Aligning the CLI *down* to the vault was chosen over upgrading the vault for that reason. **Back it up, then upgrade it — and do not raise the CLI pin until `/api/v4/secrets` answers.** Also outstanding: the `LiveKit__ApiKey`/`ApiSecret` pair and the new Infisical client secret were exposed in a working transcript and need rotation; the LiveKit pair has separately **diverged** from the LiveKit server’s own keys, so meeting joins still fail until it is rotated. Verified after the fix: replicas `1/1`, `config:` reports no `MISSING:` line, `https://api.inksphere.space/health` **200**, CORS preflight from `https://taskflow.inksphere.space` **204** with the correct allow-origin. |
| 2026-09-13 | **Both** | **First-run welcome moved to the account, the tour lock-out fixed, and assignment locks reach the dropdowns.** Four faults, one of them a hard lock-out. **(1) The welcome replayed on every sign-in.** It was remembered only in `localStorage`, so clearing site data, a private window or a second machine each read as a brand-new user. **New column** `Users.OnboardingCompletedAt` plus **new route** `POST /api/user/me/onboarding/complete` (self-scoped, idempotent) now carry it, and `GET /user/me` reports `hasCompletedOnboarding`. The migration **backfills every existing row** — that backfill, not the column, is what confines the welcome to genuinely new registrations. The client keeps its local record as a fast path only, and a session persisted by an older build (no field) reads as *seen*. **(2) The page froze until a reload.** driver.js marks `<body class="driver-active">` and its stylesheet turns that into `.driver-active * { pointer-events: none }`; nothing stopped a tour on navigation, and the welcome's third step highlights the sidebar — the one element a highlight makes clickable — so the invited click navigated, the popover was unmounted with the old page, and every click and scroll was swallowed with nothing on screen to explain it. `TourService` now tears down on `NavigationStart`, records the walk-away as a dismissal, runs `destroy()` in a `finally`, and clears driver's body classes and portalled nodes unconditionally as a backstop. **(3) `AssignTask` was not gated in the UI.** The inline assignee and team dropdowns on the tasks and project pages stayed live for a member without the permission: the pick appeared to work and the refusal arrived as a toast. `PermissionLockDirective` now covers form controls — blocking `mousedown` (what opens a select) and reverting any change that slips through, with the badge beside the control since only `<option>` may be its child. **(4) Every list filter in the app had lost its arrow.** `_toolbar.scss` set the `background` *shorthand* over `_input.scss`'s `appearance: none` caret, wiping the image; and the meetings toolbar had rolled its own controls instead of `.list-toolbar`/`.list-search`/`.list-filter`, so it rendered as bare native chrome. Both fixed. Also: the seeded admin password was `Admin@123git`, not the documented `Admin@123` — the credentials are now `Seed:Admin` configuration with the documented defaults, and `ResetPasswordOnStartup` (default **false**) is the one-boot repair for a database that already has the wrong hash. Backend **118/118**, frontend **319/319**, both builds, lint, design lint and EF drift pass. Routes **182 total**. |
| 2026-09-07 | **API** | **P7.6 — TURN/TLS live in production, and the deployment model corrected.** `turn.inksphere.space` now resolves and Traefik holds `CN=turn.inksphere.space` (to 2026-12-06), so a participant on a network permitting only `443/tcp` has a path into a meeting for the first time. The path is **proven end to end, not merely configured**: a STUN Binding Request over TLS returned a Binding Success with the transaction ID echoed, which the certificate alone would not have shown, since Traefik terminates TLS at the edge whether or not anything answers behind it. Also: `livekit.inksphere.space` kept routing throughout, and LiveKit bound `tls_port: 443` in-container as predicted. **Correction to the two rows below:** `meetings-media` uses Dokploy's **Raw provider**, not Git — so no push ever deployed it, no docs commit ever restarted LiveKit, and `infra/meetings/dokploy.compose.yml` is a reference copy that had already drifted (it carried an `egress` service production never ran). Only the LiveKit changes were applied by hand, deliberately excluding Egress from a host without the headroom for it. Traefik labels were declared under both `labels:` and `deploy.labels:` because this Traefik runs the Docker and Swarm providers, and verified not to clobber Dokploy's generated domain labels. Docs only in the repository; no code, no migration, ledger unchanged at **181 total / 178 consumed**. Remaining for P7.6: the relay `PASS` from a genuinely restrictive network, and the staged flag rollout. |
| 2026-09-07 | **API** | **P7.6 deployment follow-up — the TURN change reached production, and the deployment model is now recorded.** The P7.6 push redeployed `meetings-media` on its own: the stack is a Dokploy Compose service using the **Git provider with Auto Deploy on**, tracking `main`, so it redeploys on *any* push — a documentation-only commit included — and each redeploy restarts LiveKit and **ends every room in progress**. LiveKit returned healthy afterwards (`200 OK`, 44ms), which proves the in-container 443 bind in production. Outstanding: `turn.inksphere.space` is still `NXDOMAIN`, so Traefik serves `CN=TRAEFIK DEFAULT CERT` for that SNI and TURN/TLS is deployed but unusable until an A record points at `72.61.231.225`. Also corrected: the stack is missing from Dokploy's *sidebar* after the platform upgrade but is running normally under Projects → Taskflow → production — RUNBOOK §3 now gives the real path plus `docker stack services` / `docker service logs` as the authority, and new RUNBOOK §1b documents the auto-deploy blast radius. Docs only; no code, no migration, ledger unchanged at **181 total / 178 consumed**. |
| 2026-09-07 | **API** | **Meetings Phase 7 package P7.6 — production TURN topology and staged rollout (code complete; execution owner-gated).** Production TURN ran UDP-only with no `domain` and no TLS. [RUNBOOK §4](../infra/meetings/RUNBOOK.md) had recorded exactly that while correctly clearing TURN as the cause of the 2026-09-04 mobile fault — both true, and independent, and the clearance left the gap in place: a participant on a network permitting only `443/tcp` could reach **none** of the offered paths (UDP `7882`, TURN/UDP `3478`, ICE/TCP `7881`) and had no way into a meeting at all, with nothing in the product to explain it. `dokploy.compose.yml` now serves **TURN/TLS on 443**, terminated by Traefik and matched by SNI on `turn.inksphere.space`, with `external_tls: true`. `tls_port` is `443` rather than the conventional 5349 because LiveKit advertises `tls_port` **verbatim** as the `turns:` candidate handed to the browser — it is the port the client dials, so 5349 behind the proxy would pass every local check and fail precisely the users TURN exists for (verified against the upstream config sample and helm chart). The in-container 443 bind is safe only because `livekit/livekit-server` declares no `USER`; that dependency is recorded in the file because a future non-root image makes it a startup failure whose fix is a sysctl, not `cap_add`. New [`infra/meetings/turn-check.html`](../infra/meetings/turn-check.html) proves the result rather than asserting it: `iceTransportPolicy: 'relay'` discards host and srflx candidates, so a connection that succeeds went **through** TURN, and the harness separately reports the advertised `iceServers` (captured by wrapping `RTCPeerConnection`, not by reading livekit-client internals) so "no `turns:443` offered" and "the relay failed" read as different faults; it emits a copyable evidence block. New [`infra/meetings/ROLLOUT.md`](../infra/meetings/ROLLOUT.md) covers provisioning order with certificate verification, the Dokploy File Mount that closes RUNBOOK §2's still-open configuration-persistence gap (written with all three `Meetings__*` flags **false**, so the fallback floor is the safe state), and the `Enabled` → `GuestsEnabled` → `RecordingEnabled` staging with per-stage verification and rollback triggers — including that rolling back via `LiveKit__Enabled` is wrong, since it leaves meetings listed and joinable-looking while every join refuses, the 2026-09-02 failure exactly. **No backend or frontend code changed, no migration, no route change** — ledger unchanged at **181 total / 178 consumed**. The harness was exercised locally against an unreachable endpoint (failure path, per-check reporting, verdict and evidence capture all render); **a `PASS` requires the host**, as do DNS, the redeploy, the certificate, the File Mount and the staged flag raises. |
| 2026-09-06 | **Both** | **Meetings Phase 7 package P7.7 — privacy, retention, support and operations.** Added [MEETINGS-PRIVACY.md](MEETINGS-PRIVACY.md) (a per-store inventory of what one meeting holds, which columns are personal data, how the retention clock works, what leaves TaskFlow and to whom, and how to answer a data-subject request), [MEETINGS-SUPPORT.md](MEETINGS-SUPPORT.md) (symptom → error code → who can fix it, for members, guests, collaboration and recording, with an escalation boundary into the RUNBOOK) and [infra/meetings/OPERATIONS.md](../infra/meetings/OPERATIONS.md) (what to back up and in which order, what a restore does *not* give back, per-secret rotation blast radius, and the incident procedure with its containment levers). **Frontend:** the public Privacy Policy gained a **Meetings, calls and recordings** section, a meeting-specific retention paragraph, and named provider kinds under sharing. Writing the documents against the code established six previously unstated facts, all now recorded rather than smoothed over: retention **never touches `MeetingParticipants` or `MeetingAccessLinks`**, so guest email addresses and invited addresses outlive the window indefinitely; `SoftDelete()` nulls no column, so chat and note bodies remain in PostgreSQL after retention; only `Ended` meetings with an `ActualEndUtc` are ever swept, leaving cancelled and abandoned meetings permanent; only `Ready` recordings have their object erased; a storage outage stalls the sweep silently; and rotating `JwtSettings:SecretKey` also invalidates every pending guest OTP because `OneTimeCodeSettings:SecretKey` falls back to it. The policy is worded to that reality; closing the gaps is proposed as **P7.8** and is not required for Phase 7. Routes unchanged at **181 total / 178 consumed**; no backend change, **no migration**. Frontend typecheck, production build, **291/291** specs, lint, design lint and 42 contrast checks pass. The OPERATIONS.md procedures are **written but not yet exercised** — no restore or rotation drill has been run. |
| 2026-09-13 | **UI** | **Subtasks are visible and editable on the organization project page.** The task row showed only a `done/total` count, so breaking an imported task down meant leaving the project for the tasks page; the count is now an expander that opens the checklist in place with tick-off, delete and an add box. **No API change** — it binds the existing `GET /subtask/task/{id}`, `POST /subtask`, `PUT /subtask/{id}/complete|reopen` and `DELETE /subtask/{id}`, which the tasks page already used. Write controls are gated on the existing `canManageTasks` (owner bypass included); reading is open to anyone who can open the project. **Known gap, accepted:** that gate is an affordance only — the API gates subtasks on membership (`EnsureTaskAsync`), not on a permission, so a member without `ManageTasks` is refused by the UI but would be allowed by a direct API call. Closing it needs a permission check in the four subtask handlers. Fixed in passing: a `data-tour` attribute committed **inside** a calendar heading's text (rendered raw to users, failed a spec) and an unused import failing `ng lint` on main. Frontend **307/307**, build, lint, design lint and 42 contrast checks pass. |
| 2026-09-13 | **UI** | **Markdown project plans can be imported, alongside CSV.** `@shared/utils/project-plan-markdown` reads a plan written as headings and bullets — the first heading is the project, each `##` heading a task, `Field: value` bullets set dates, priority, estimate, team and assignee, and any other bullet is a subtask — and produces the same `ProjectPlanImportPayload` the CSV reader produces, so **`POST /project/plan-import` and the whole backend are untouched**. Both projects pages now offer a **Markdown template** download beside the CSV one and accept `.md` in the import drawer. The reader ignores everything it does not recognise — HTML comments, fenced code, blockquotes and sections titled Instructions / Notes / Examples / Guidance — which is what lets the downloaded template carry its own instructions and still import correctly if the user leaves them in. Defaults keep a minimal plan valid: task start falls back to the project start, priority to Medium, key to `T-001`. The same 500-task / 5 000-subtask / 5 MB limits and date-range rule are enforced before the request is sent. Routes unchanged; Angular build and 9 plan-reader specs pass. **No migration.** |
| 2026-09-05 | **Both** | **Meetings Phase 7 package P7.4 — structured metrics, traces and logs with alerts and runbooks.** One meter (`TaskFlow.Meetings`) now carries edge requests and latency, join-token issuance and refusal, the guest verification funnel, webhook outcomes, the recording lifecycle, capacity refusals and every outbound LiveKit call with its duration — chosen because each is a failure the UI keeps looking healthy through: a rejected webhook stops attendance being written while the archive still renders, and a recording that never started leaves the host believing it did. Eight alert rules with thresholds, windows, severities and a runbook each are in [MEETINGS-OBSERVABILITY.md](MEETINGS-OBSERVABILITY.md). The privacy contract is enforced by a test rather than documented: no email, access-link or session token, join token, room name, participant identity, display name, title, chat text, file name or IP may become a tag or log field, and no identifier may become a **metric** tag (the edge middleware tags the route template and buckets unmatched paths, so a caller cannot mint series by inventing URLs) — identifiers live on the span, so metrics say *that* something is failing and the trace says *which meeting*. **New route:** `GET /admin/meetings/health` (AdminOnly) evaluates the rules in-process over a bounded one-hour ring, for a deployment with no metrics collector; it reports its own limits — the window is per instance and `fullyObserved` is false until the process has collected for a full hour. **Frontend:** a **Meetings health** panel beside Meetings readiness on `/admin/settings`, showing firing alerts with their observation, threshold and runbook anchor, plus 5 / 15 / 60-minute counts per signal. Two defects that only real HTTP exposes were found and fixed on the way: the snapshot was built lazily on the first read of the endpoint, so it reported an empty window over an API that had been serving meetings for hours (it is a hosted service now), and refusals thrown as exceptions passed the middleware with the response status still at its default 200, counting every refused meeting request as a healthy one (`ExceptionHandlingMiddleware.StatusCodeFor` is now the single mapping both middlewares read). Routes **181 total / 178 consumed**; backend **113/113**, frontend **291/291**, builds, lint, design lint, 42 contrast checks and EF drift pass. **No migration.** Not covered, and said so in the doc: client-side media quality (packet loss, jitter, ICE, TURN) lives in the browser and in LiveKit. |
| 2026-09-05 | **Both** | **Meetings Phase 7 package P7.3 — declared capacity and concurrency.** Meetings now declare limits they can defend instead of implying unlimited scale, and every ceiling is refused server-side with a code naming the number: 50 participants per meeting, 10 live meetings per organization, 1 simultaneous recording per deployment, 5 000 messages and 100 files (25 MB each, 250 MB total) per meeting — all `Meetings:Max*` configuration, validated at startup and documented in [MEETINGS-CAPACITY.md](MEETINGS-CAPACITY.md). The file quota replaces an implicit `MaxFileBytes * 10` inside the upload handler. One real concurrency defect was fixed: chat duplicate-suppression read before it wrote, so two retries in flight at once both wrote, the unique index refused one, and that caller was told their message failed while it was in the room; the loser now re-reads and reports the winner, proved by six simultaneous retries against a real database. Threat-model residuals A-07 (guest sessions/OTP challenges never purged) and A-08 (one shared 12/minute guest budget, now three keyed by session) are closed. **Frontend:** `GET /admin/meetings/readiness` gained a `capacity` object and the admin readiness panel renders it as **Declared capacity**, so an operator can size a deployment or explain a refusal without reading a config file. Routes unchanged at **180 total / 177 consumed**; backend **98/98**, frontend **287/287**, builds, lint, design lint, 42 contrast checks and EF drift pass. **No migration** — capacity is configuration. |
